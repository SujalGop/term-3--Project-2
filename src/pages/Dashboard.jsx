...
import { RiBookOpenLine, RiTaskLine, RiCheckboxCircleLine, RiTimeLine } from 'react-icons/ri';
...
export default function Dashboard() {
  ...
  const pieData = [
    { name: 'Completed',         value: topics.filter(t => t.status === 'Completed').length || 1 },
    { name: 'Pending Revisions', value: topics.filter(t => t.nextRevisionDate && new Date(t.nextRevisionDate) <= today).length },
    { name: 'Upcoming Revisions', value: topics.filter(t => t.nextRevisionDate && new Date(t.nextRevisionDate) > today).length },
  ].filter(d => d.value > 0);
        ...
        <Card animate={false} className="lg:col-span-2">
          ...
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={weeklyData} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
              ...
              <Tooltip contentStyle={{ background: '#1e293b', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', color: '#f1f5f9', fontSize: 12 }} cursor={{ stroke: 'rgba(255,255,255,0.1)' }} />
        ...
        <Card animate={false}>
            ...
            {progressBySubject.map(({ subjectId, name, icon, color, percentage }) => (
              <div key={subjectId}>
                <div className="flex justify-between items-center mb-1.5">
                  <span className="text-xs font-medium text-surface-300 flex items-center gap-1.5"><span>{icon}</span> {name}</span>
                ...
                <div className="w-full h-1.5 bg-surface-700/70 rounded-full overflow-hidden">
                  <motion.div className="h-full rounded-full" style={{ backgroundColor: color }} initial={{ width: 0 }} animate={{ width: `${percentage}%` }} transition={{ duration: 0.8, delay: 0.2, ease: 'easeOut' }} />
                </div>
              </div>
            ))}
          </div>
          <div className="mt-5 pt-4 border-t border-white/10">
            ...
            <div className="w-full h-2 bg-surface-700/70 rounded-full overflow-hidden">
              <motion.div className="h-full bg-gradient-to-r from-primary-500 to-accent-500 rounded-full" initial={{ width: 0 }} animate={{ width: `${overallPercentage}%` }} transition={{ duration: 0.9, delay: 0.3, ease: 'easeOut' }} />
            </div>
          </div>
        </Card>
      </div>

      {/* Upcoming revisions + pie chart */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card animate={false} className="lg:col-span-2">
