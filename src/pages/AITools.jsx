    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {TOOLS.map(tool => (
          <button
            ...
            className={`text-left glass rounded-2xl p-4 transition-all duration-200 ${activeTool === tool.id ? 'border border-primary-500/50 bg-primary-600/10' : 'glass-hover'}`}
          >
      ...
      <Card animate={false}>
        <div className="space-y-3">
          ...
          {activeTool === 'summary' ? (
            <textarea rows={6} placeholder="Paste your study notes or any text here…" value={text} onChange={e => setText(e.target.value)} className="form-input resize-none" />
          ) : (
            <div>
              <input placeholder={`Enter a topic (e.g. "Binary Search Trees")…`} value={topic} onChange={e => setTopic(e.target.value)} className="form-input" />
              {subjectTopics.length > 0 && (
                <div className="mt-3">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {subjectTopics.map(t => (
                      <button key={t.id} onClick={() => setTopic(t.title)} className="text-[11px] bg-primary-500/10 text-primary-300 px-3 py-1.5 rounded-md border border-primary-500/20 hover:bg-primary-500/20 hover:border-primary-500/40 transition-colors">
                        {t.title}
                      </button>
                    ))}
                  </div>
      ...
      {/* Error message */}
      {error && <div className="glass border border-red-500/30 rounded-2xl p-4 text-red-400 text-sm">⚠️ {error}</div>}
                      ...
                      <div key={i} className="glass rounded-xl p-4">
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-2">
                          {q.options?.map((opt, j) => {
                            ...
                            const isCorrect     = optLower === correctAnswer;
                            const isSelected    = optLower === selected?.trim().toLowerCase();
                            let btnClass = 'bg-white/5 text-surface-300 hover:bg-white/10 cursor-pointer';
                            if (isAnswered) {
                              if (isCorrect)     btnClass = 'bg-accent-500/20 text-accent-300 border border-accent-500/40';
                              else if (isSelected) btnClass = 'bg-red-500/20 text-red-300 border border-red-500/40';
                              else                 btnClass = 'bg-white/5 text-surface-500 opacity-50 cursor-default';
                            }
                            return (
                              <button key={j} onClick={() => !isAnswered && setUserAnswers(prev => ({ ...prev, [i]: opt }))} disabled={isAnswered} className={`text-xs text-left rounded-lg px-3 py-2 transition-all ${btnClass}`}>
                                {opt}
                              </button>
                            );
                          })}
