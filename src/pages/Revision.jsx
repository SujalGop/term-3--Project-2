/**
 * Revision.jsx
 * Full-page calendar highlighting dates with scheduled revisions and tasks.
 */
...
export default function Revision() {
  ...
  const handleTileContent = ({ date, view }) => {
    if (view === 'month') {
      const dayTopics = topics.filter(t => 
        t.nextRevisionDate &&
        new Date(t.nextRevisionDate).setHours(0,0,0,0) === date.setHours(0,0,0,0)
      );
      
      const dayTasks = tasks.filter(tsk => 
        tsk.dueDate && !tsk.completed &&
        new Date(tsk.dueDate).setHours(0,0,0,0) === date.setHours(0,0,0,0)
      );

      const itemsCount = dayTopics.length + dayTasks.length;

      if (itemsCount > 0) {
        return (
          <div className="mt-1 flex justify-center gap-1 flex-wrap max-w-full">
            {dayTopics.slice(0, 2).map((_, i) => (
              <div key={`top-${i}`} className="w-1.5 h-1.5 rounded-full bg-primary-500"></div>
            ))}
            {dayTasks.slice(0, 2).map((_, i) => (
              <div key={`tsk-${i}`} className="w-1.5 h-1.5 rounded-full bg-amber-500"></div>
            ))}
            {itemsCount > 4 && <div className="w-1.5 h-1.5 rounded-full bg-surface-400"></div>}
          </div>
        );
      }
    }
  ...
  const activeTopicDates = topics.filter(t => t.nextRevisionDate).map(t => new Date(t.nextRevisionDate).setHours(0,0,0,0));
  const activeTaskDates = tasks.filter(t => t.dueDate && !t.completed).map(t => new Date(t.dueDate).setHours(0,0,0,0));

  const handleTileClassName = ({ date, view }) => {
    if (view === 'month') {
      const curDate = date.setHours(0,0,0,0);
      const hasTopic = activeTopicDates.includes(curDate);
      const hasTask = activeTaskDates.includes(curDate);
      
      if (hasTopic && hasTask) return 'highlighted-date-both';
      if (hasTopic) return 'highlighted-date';
      if (hasTask) return 'highlighted-date-task';
    }
    return null;
  };

  // Filter components based on selectedDate
  const displayTopics = selectedDate
    ? topics.filter(t => t.nextRevisionDate && new Date(t.nextRevisionDate).setHours(0,0,0,0) === selectedDate.setHours(0,0,0,0))
  ...
  const displayTasks = selectedDate
    ? tasks.filter(tsk => tsk.dueDate && !tsk.completed && new Date(tsk.dueDate).setHours(0,0,0,0) === selectedDate.setHours(0,0,0,0))
    : tasks.filter(tsk => tsk.dueDate && !tsk.completed).sort((a,b) => new Date(a.dueDate) - new Date(b.dueDate)).slice(0, 4);
        ...
        <div className="lg:col-span-2">
          <Card animate={false} className="p-2 sm:p-6 overflow-hidden">
            <style>{`
              ...
              .highlighted-date-task {
                background-color: rgba(245, 158, 11, 0.1) !important;
                border: 1px solid rgba(245, 158, 11, 0.3) !important;
              }
              .highlighted-date-both {
            ...
            <Calendar 
              ...
              value={selectedDate}
              className="w-full text-white pb-4"
            />
          </Card>
        </div>
            ...
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="text-base font-semibold text-white">
                  {selectedDate ? 'Schedule for' : 'Upcoming Schedule'}
                </h3>
                {selectedDate && (
                  <p className="text-xs text-primary-400 mt-0.5">{formatDate(selectedDate)}</p>
                )}
              </div>
              {selectedDate && (
                <button 
                  onClick={() => setSelectedDate(null)}
                  className="text-[10px] text-surface-400 hover:text-white px-2 py-1 bg-surface-700/50 rounded"
                >
                  Clear
                </button>
              )}
