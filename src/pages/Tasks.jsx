/**
 * Tasks.jsx
 * Top-level Tasks page equipped with advanced filtering, sorting, and search.
 */
...
import {
  RiAddLine, RiDeleteBin6Line, RiCheckboxCircleLine,
  RiCheckboxBlankCircleLine, RiEdit2Line
} from 'react-icons/ri';
...
export default function Tasks() {
  ...
  // Advanced feature pipe: Apply global search + filters, then apply sort
  const processedTasks = sortData(
    filterTasks(tasks, subjects, topics, searchQuery, filters),
    sortBy,
    subjects
  );
              ...
              >
                <Card animate={false} className={`flex items-center gap-4 py-3.5 px-4 ${task.completed ? 'opacity-60' : ''}`}>
                  ...
                  <button onClick={() => toggleTask(task.id)} className="shrink-0 text-2xl transition-colors text-surface-500 hover:text-accent-400">
                    {task.completed
                      ? <RiCheckboxCircleLine className="text-accent-400" />
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                       <p className={`text-sm font-medium truncate ${task.completed ? 'line-through text-surface-500' : 'text-white'}`}>
                         {task.title}
                       </p>
                       {parentSubject && (
                  ...
                  {/* Priority Badge */}
                  <span className={`badge ${priority.badge} shrink-0 hidden sm:inline-flex`}>
                    {task.priority}
                  </span>

                  {/* Actions */}
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => handleEdit(task)} className="text-surface-600 hover:text-primary-400 transition-colors p-1" title="Edit Task">
                      <RiEdit2Line />
                    </button>
                    <button onClick={() => deleteTask(task.id)} className="text-surface-600 hover:text-red-400 transition-colors p-1" title="Delete Task">
                      <RiDeleteBin6Line />
                    </button>
                  </div>
                </Card>
