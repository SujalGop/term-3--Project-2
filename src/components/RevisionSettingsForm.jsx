...
export default function RevisionSettingsForm() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      toast.success(`Revision scheduled for ${data.nextRevisionDate}`);
      reset();
    } catch (err) {
      ...
      <div>
        ...
        <div className="flex gap-3 relative">
          <div className="flex-1">
            <input {...register('nextRevisionDate')} type="date" className="form-input w-full" />
            {errors.nextRevisionDate && (
              <p className="text-red-400 text-xs mt-1 absolute -bottom-5 left-0">{errors.nextRevisionDate.message}</p>
            )}
          </div>
          <Button type="submit" disabled={isSubmitting}>{isSubmitting ? 'Saving...' : 'Set Date'}</Button>
        </div>
      </div>
    </form>
  );
