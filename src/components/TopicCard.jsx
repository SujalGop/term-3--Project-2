...
export default function TopicCard({ topic, onDelete, onStatusChange, onUpdate }) {
  ...
  // Coerce legacy notes gracefully
  ...
  const handleSaveTopic = () => {
    if (onUpdate) {
      // Ensure we keep the notes array unmodified by the top-level edit form which used to destroy it
      onUpdate({...editForm, notes: topic.notes});
    }
    setIsEditing(false);
  };
  ...
  const saveNewNote = () => {
    ...
    const added = [...notesArray, { id: Date.now().toString(), title: newNote.title, content: newNote.content }];
    if (onUpdate) onUpdate({ ...topic, notes: added });
    setNewNote({ title: '', content: '' });
    setAddingNote(false);
  };

  const deleteNote = (noteId) => {
    const reduced = notesArray.filter(n => n.id !== noteId);
    if (onUpdate) onUpdate({ ...topic, notes: reduced });
  };

  const saveEditNote = () => {
    const modified = notesArray.map(n => n.id === editingNoteId ? { ...n, ...editNoteData } : n);
    if (onUpdate) onUpdate({ ...topic, notes: modified });
        ...
        <div className="flex flex-col gap-3">
          <div className="flex justify-between items-center bg-surface-800 p-2 rounded border border-surface-700">
             ...
             <div className="flex gap-2">
               <button onClick={handleSaveTopic} className="text-emerald-400 hover:text-emerald-300 p-1" title="Save">
                 <RiCheckLine size={18} />
               </button>
               <button onClick={() => setIsEditing(false)} className="text-surface-400 hover:text-surface-300 p-1" title="Cancel">
                 <RiCloseLine size={18} />
               </button>
             </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs mt-1">
             ...
             {/* Removed legacy Notes textarea from top-level edit form to prevent collision */}
          </div>
        </div>
      </Card>
    );
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button
              onClick={() => { setEditForm(topic); setIsEditing(true); }}
              className="text-surface-500 hover:text-primary-400 transition-colors p-1"
              title="Edit Topic & Revision Schedule"
            >
              <RiEdit2Line size={16} />
            </button>
            <button
        ...
        {/* Badges row: Difficulty and Status */}
        <div className="flex items-center gap-3 text-xs">
          {topic.difficulty && (
            <span className={`px-2 py-0.5 rounded border ${DIFFICULTY_COLORS[topic.difficulty] || 'text-surface-400'}`}>
              {topic.difficulty}
            </span>
          )}
          ...
          >
            {STATUS_OPTIONS.map(status => (
              <option key={status} value={status} className="bg-surface-800 text-white">
                {status}
              </option>
            ))}
          </select>
              ...
              >
                  ...
                  <div key={note.id} className="bg-surface-800/50 rounded border border-surface-700/30 overflow-hidden">
                    {editingNoteId === note.id ? (
                      <div className="p-2 space-y-2">
                        <input value={editNoteData.title} onChange={e => setEditNoteData({...editNoteData, title: e.target.value})} className="form-input p-1 text-xs" placeholder="Note Title (e.g. Class 1)" />
                        <textarea value={editNoteData.content} onChange={e => setEditNoteData({...editNoteData, content: e.target.value})} className="form-input p-1.5 text-xs resizer-none" rows={3} placeholder="Content..."></textarea>
                ...
                {addingNote ? (
                  <div className="bg-surface-800/80 rounded border border-primary-500/20 p-2 space-y-2 mt-2">
                    <input value={newNote.title} onChange={e => setNewNote({...newNote, title: e.target.value})} className="form-input p-1 text-xs" placeholder="New Note Title (e.g. 'Class 2 Notes')" />
                    <textarea value={newNote.content} onChange={e => setNewNote({...newNote, content: e.target.value})} className="form-input p-1.5 text-xs resizer-none" rows={3} placeholder="Paste notes here..."></textarea>
                    <div className="flex gap-2 justify-end">
