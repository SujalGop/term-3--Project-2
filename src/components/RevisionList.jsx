...
export default function RevisionList() {
  ...
  // Find topics that have a scheduled revision
    ...
    >
      <AnimatePresence>
        {scheduledTopics.map(topic => {
          ...
          const isOverdue = new Date(topic.nextRevisionDate).setHours(0,0,0,0) < new Date().setHours(0,0,0,0);
          const isToday = new Date(topic.nextRevisionDate).setHours(0,0,0,0) === new Date().setHours(0,0,0,0);
          
          let dateColor = 'text-surface-400';
          if (isOverdue) dateColor = 'text-red-400 font-semibold';
          else if (isToday) dateColor = 'text-amber-400 font-semibold';
