...
export default function FilterSortPanel() {
  ...
  const onFilter = (field, value) => actions.setFilters({ [field]: value });
  const onSortField = (field) => actions.setSortBy({ ...sortBy, field });
  const toggleDirection = () => actions.setSortBy({ ...sortBy, direction: sortBy.direction === 'asc' ? 'desc' : 'asc' });
  ...
  return (
    <div className="w-full bg-surface-800/40 border border-surface-700/50 rounded-xl p-4 flex flex-col xl:flex-row gap-4 items-start xl:items-center justify-between shadow-sm">
      ...
      <div className="flex flex-wrap items-center gap-3 flex-1 flex-col sm:flex-row w-full xl:w-auto">
        ...
        <select value={filters.subject}  onChange={e => onFilter('subject', e.target.value)}  className="form-input text-xs py-1.5 focus:ring-1 bg-surface-800 w-full sm:w-auto">
          <option value="all">All Subjects</option>
          {subjects.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
        </select>
        <select value={filters.priority} onChange={e => onFilter('priority', e.target.value)} className="form-input text-xs py-1.5 focus:ring-1 bg-surface-800 w-full sm:w-auto">
          <option value="all">All Priorities</option>
          <option value="high">High Priority</option>
          <option value="medium">Medium Priority</option>
          <option value="low">Low Priority</option>
        </select>
        <select value={filters.status}   onChange={e => onFilter('status', e.target.value)}   className="form-input text-xs py-1.5 focus:ring-1 bg-surface-800 w-full sm:w-auto">
          <option value="all">All Statuses</option>
          <option value="pending">Pending</option>
          <option value="completed">Completed</option>
        </select>
        <select value={filters.deadline} onChange={e => onFilter('deadline', e.target.value)} className="form-input text-xs py-1.5 focus:ring-1 bg-surface-800 w-full sm:w-auto">
      ...
      <div className="flex flex-wrap items-center gap-2 w-full xl:w-auto justify-between sm:justify-start">
        ...
        <select value={sortBy.field} onChange={e => onSortField(e.target.value)} className="form-input text-xs py-1.5 focus:ring-1 bg-surface-800 flex-1 sm:flex-none">
