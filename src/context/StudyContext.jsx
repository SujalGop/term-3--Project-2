...
// Generate a unique ID using the browser crypto API or Math.random as fallback
const genId = () =>
...
export const ACTIONS = {
  ADD_SUBJECT: 'ADD_SUBJECT', UPDATE_SUBJECT: 'UPDATE_SUBJECT', DELETE_SUBJECT: 'DELETE_SUBJECT',
  ADD_TOPIC:   'ADD_TOPIC',   UPDATE_TOPIC:   'UPDATE_TOPIC',   DELETE_TOPIC:   'DELETE_TOPIC',
  ADD_TASK:    'ADD_TASK',    UPDATE_TASK:    'UPDATE_TASK',    DELETE_TASK:    'DELETE_TASK',    TOGGLE_TASK: 'TOGGLE_TASK',
  SET_LOADING: 'SET_LOADING', SET_ERROR: 'SET_ERROR', CLEAR_ERROR: 'CLEAR_ERROR',
  SET_SEARCH_QUERY: 'SET_SEARCH_QUERY', SET_FILTERS: 'SET_FILTERS', SET_SORT_BY: 'SET_SORT_BY',
...
function studyReducer(state, action) {
  switch (action.type) {
    ...
    case ACTIONS.ADD_SUBJECT:
      return { ...state, subjects: [...state.subjects, { ...action.payload, id: genId(), createdAt: now() }] };
    ...
    case ACTIONS.DELETE_SUBJECT: {
      const deletedTopicIds = state.topics.filter(t => t.subjectId === action.payload).map(t => t.id);
      return {
        ...state,
    ...
    case ACTIONS.ADD_TOPIC:
      return { ...state, topics: [...state.topics, { ...action.payload, id: genId(), createdAt: now() }] };
    ...
    case ACTIONS.UPDATE_TOPIC: {
      ...
      const updatedTopics = state.topics.map(t => {
        ...
        if (isCompleting && t.status !== 'Completed') updated.completedAt = now();
        return updated;
      });
    ...
    case ACTIONS.ADD_TASK:
      return { ...state, tasks: [...state.tasks, { ...action.payload, id: genId(), completed: false, createdAt: now() }] };
    ...
    case ACTIONS.SET_FILTERS:           return { ...state, filters: { ...state.filters, ...action.payload } };
    case ACTIONS.SET_SORT_BY:           return { ...state, sortBy: action.payload };
    case ACTIONS.SET_REVISION_SETTINGS: return { ...state, revisionSettings: { ...state.revisionSettings, ...action.payload } };
